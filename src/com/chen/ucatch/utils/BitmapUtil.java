...
public final class BitmapUtil {
	...
	public static byte[] getHttpImageByte(final String url) {
		...
		try {
			final HttpURLConnection conn = (HttpURLConnection) myFileUrl
					.openConnection();
	 ...
	 * @return 返回相应的绝对路径
	 */
	public static String getUriImagePathQuery(final Context ct,
		...
		String path = "";
		final Cursor cursor = ct.getContentResolver().query(contentUri, proj,
				null, null, null);
		if (cursor != null) {
	 ...
	 * @return 图片流
	 */
	public static ByteArrayInputStream getInputStreamFromBitmap(
			final Bitmap bitmap) {
		try {
	 ...
	 * @return 图片
	 */
	public static Bitmap getImageFromAssetFile(final Context context,
			final String fileName) {
		return getImageFromAssetFile(context, fileName,
	 ...
	 *            文件相对路劲
	 * @param density
	 *            Options.inDensity的设置,在一些情况下,需要制定.否则,会变形
	 * @return 返回图片
	 */
	public static Bitmap getImageFromAssetFile(final Context context,
			final String fileName, final int density) {
	 ...
	 * @return 缩放后图片
	 */
	public static Bitmap createScaledBitmap(final Bitmap bmp,
			final int mScreenWidth, final int mScreenHeight) {
		return Bitmap
				.createScaledBitmap(bmp, mScreenWidth, mScreenHeight, true);
	}
	...
	public static Bitmap createReflectionImageWithOrigin(final Bitmap bitmap) {
		...
		final Bitmap reflectionImage = Bitmap.createBitmap(bitmap, 0, h / 2, w,
				h / 2, matrix, false);

		final Bitmap bitmapWithReflection = Bitmap.createBitmap(w, h + h / 2,
				Config.ARGB_8888);
		...
		final Paint paint = new Paint();
		final LinearGradient shader = new LinearGradient(0, bitmap.getHeight(),
	 ...
	 * @return selector图片
	 */
	public static StateListDrawable getNnewSelector(final Drawable defPic,
			final Drawable selPic) {
	 ...
	 */
	public static StateListDrawable getNewSelector(final Drawable normal,
			final Drawable pressed, final Drawable focused,
			final Drawable unable) {
	 ...
	 * @return 返回selector 的对象布局
	 */
	public static StateListDrawable getRadioButtonSelector(
			final Drawable normal, final Drawable selected,
			final Drawable pressed) {
		final StateListDrawable drawable = new StateListDrawable();
		drawable.addState(new int[] { R.attr.state_focused,
				android.R.attr.state_enabled }, pressed);
		drawable.addState(new int[] { R.attr.state_pressed,
				android.R.attr.state_enabled }, normal);
		drawable.addState(new int[] { R.attr.state_checked,
				android.R.attr.state_enabled }, pressed);
		drawable.addState(new int[] { R.attr.state_selected,
				android.R.attr.state_enabled }, pressed);
	 ...
	 * @return Bitmap
	 */
	// public static Bitmap getBitmapCompression(final String path, final int
	// swidth, final int quality) {
	...
	public static Bitmap compressImage(final Bitmap image) {
		...
		final ByteArrayInputStream isBm = new ByteArrayInputStream(
	 ...
	 *            路径
	 * @param swidth
	 *            压缩的宽度比例 0以下标识不压缩图片,只压缩质量
	 * @param quality
	 *            质量
	 * @return Bitmap
	 */
	public static Bitmap getBitmapCompression(final String srcPath,
			float swidth, final int quality) {
	 ...
	 * @return Bitmap
	 */
	public static Bitmap getBitmapCompression(final Bitmap image,
			final float swidth) {
	 ...
	 */
	public static Bitmap getRoundedCornerBitmap(final Bitmap outBitmap1,
			final Bitmap bitmap) {
		final int sm = 0;
		...
		final int color = 0xff424242;
		final Paint paint = new Paint();
		final Rect rect = new Rect(sm, sm, bitmap.getWidth() - sm,
				bitmap.getHeight() - sm);
		final RectF rectF = new RectF(rect);
	...
	/*	
	*//**
	 * 压缩指定路径下图片 想要图片大小成屏幕大小时只传context，想要图片大小和imageView一样时只穿imageView
	 * 允许image或context为null或都为null，都为null时会返回原图，有一定几率oom
	 * 
	 * @author wangguanwen
	 * @param filepath
	 *            图片路径
	 ...
	 */
	/*
	 * public static Bitmap getCompressedBitmapFromFile(final String filepath,
	 * final ImageView imageView, Context context) { BitmapFactory.Options
	 * options = new BitmapFactory.Options(); options.inJustDecodeBounds = true;
	 * BitmapFactory.decodeFile(filepath, options); final int srcHeight =
	 * options.outHeight; // 原图高 final int srcWidth = options.outWidth; // 原图宽
	 * String imageType = options.outMimeType; // 图片类型 // Log.i("源图片信息:", "高:" +
	 * srcHeight + "," + "宽:" + srcWidth + "," + "格式:" + imageType);
	 * 
	 * options.inSampleSize = 1; // 默认压缩比例1 int imageViewH = 0; // 控件高 int
	 * imageViewW = 0; // 控件宽 if (context == null) { context =
	 * BaseApplication.getApplication(); } DisplayMetrics dm = new
	 * DisplayMetrics(); dm = context.getResources().getDisplayMetrics(); int
	 * screenWidth = dm.widthPixels; // 屏幕宽（像素，如：480px） int screenHeight =
	 * dm.heightPixels; // 屏幕高（像素，如：800px） imageViewH = screenHeight; imageViewW
	 * = screenWidth;
	 * 
	 * if (imageView != null) { if (imageView.getHeight() > 0) { imageViewH =
	 * imageView.getHeight(); // 控件高 } if (imageView.getWidth() > 0) {
	 * imageViewW = imageView.getWidth(); // 控件宽 } }
	 * 
	 * // Log.i("imageView控件信息:", "控件高:" + imageViewH + "," + "控件宽:" +
	 * imageViewW);
	 * 
	 * if (imageViewH > 0 && imageViewW > 0) {
	 * 
	 * if (srcHeight > imageViewH || srcWidth > imageViewW) { // 计算高宽比 final int
	 * heightRatio = Math.round((float) srcHeight / (float) imageViewH); final
	 * int widthRatio = Math.round((float) srcWidth / (float) imageViewW);
	 * options.inSampleSize = heightRatio > widthRatio ? heightRatio :
	 * widthRatio; // 压缩比例 // Log.i("高宽比:", "高:" + heightRatio + "," + "宽:" +
	 * widthRatio); } } options.inJustDecodeBounds = false;
	 * 
	 * Bitmap bitmap = BitmapFactory.decodeFile(filepath, options); //
	 * Log.i("压缩后", "高:" + bitmap.getHeight() + "," + "宽:" + bitmap.getWidth());
	 * 
	 * return bitmap; }
	 */
	/**
	 ...
	 * 进行压缩,先以比例压缩,然后精致压缩到等比距离,这种压缩方法,可以很大程度的降低内存溢出 . <br>
	 * 
	 * @author chenjiqiang 2016年3月19日 下午1:37:34 <br>
	 * @param srcPath
	 * @param swidth
	 * @param quality
	 * @return
	 */
	public static Bitmap getBitmapCompres(final String srcPath, float swidth,
			final int quality) {
		final BitmapFactory.Options newOpts = new BitmapFactory.Options();
		// 开始读入图片，此时把options.inJustDecodeBounds 设回true了
		newOpts.inJustDecodeBounds = true;
